  ...
  int main() {
    ...
    for (int i = 0; i < expr.length(); i++) {
      ...
      // Closing bracket
        ...
        if (st.empty()) {
          cout << "Error: Extra closing bracket '" << c << "' at position " << i
               << endl;
          return 0;
        }
        ...
        if (topChar != match[c]) {
          ...
          cout << "Expected '" << reverseMatch[topChar] << "' but found '" << c
               << "'" << endl;
          cout << "Opening bracket '" << topChar << "' was at position "
               << topPos << endl;
          return 0;
        }
    ...
    if (!st.empty()) {
      ...
      while (!st.empty()) {
        cout << "Unclosed '" << st.top().first << "' at position "
             << st.top().second << endl;
        st.pop();
      }
      return 0;
